...
module Css4.Selector.Core (
    ...
    , CaseSensitivity(CaseSensitive, CaseInsensitive)
    ...
    , PseudoClass(
        ...
        , Link, NthChild, NthCol, NthLastChild, NthLastCol, NthLastOfType, NthOfType, OnlyOfType, OnlyChild, Optional, OutOfRange
        , ReadOnly, ReadWrite, Required, Root, Target, Valid, Visited
...
import Css4.Selector.Utils(encodeIdentifier, encodeText, toIdentifier)
...
data SelectorCombinator =
    ...
    | Column -- ^ The first selector determines the column, whereas the second element determines the cell, denoted with @||@ in css.
...
-- space for 'Descendant', a @>@ for 'Child', a @+@ for 'DirectlyPreceded',
-- a @~@ for 'Preceded', and a @||@ for 'Column'.
...
-- | Combines two 'Selector's with the 'Column' combinator.
(.||) :: Selector -- ^ The left 'Selector'.
    -> Selector -- ^ The right 'Selector'.
    -> Selector -- ^ A selector that is the combination of the left 'Selector' and the right 'Selector' through 'Column'.
(.||) = combine Column

-- | Since Css-4, one can match values in a case-sensitive and case-insensitive way. This is done with a @i@ or @s@ at the end of the attribute selector.
data CaseSensitivity
  = CaseSensitive  -- ^ The matching is case-sensitive, this is denoted with an @s@ in the attribute selector.
  | CaseInsensitive  -- ^ The matching is case-insensitive, this is denoted with an @i@ in an attribute selector.
  deriving (Bounded, Data, Enum, Eq, Generic, Ord, Read, Show)
...
instance ToCssSelector PseudoClass where
    toCssSelector = cons ':' . go
      where go Active = "active"
            ...
            go (NthCol nth) = "nth-col(" <> nthToText nth <> ")"
            go LastChild = "last-child"
            go (NthLastChild nth) = "nth-last-child(" <> nthToText nth <> ")"
            go (NthLastCol nth) = "nth-last-col(" <> nthToText nth <> ")"
    ...
    toPattern (NthCol nth) = _conP 'NthCol [_nthToPat nth]
    toPattern (NthLastChild nth) = _conP 'NthLastChild [_nthToPat nth]
    toPattern (NthLastCol nth) = _conP 'NthLastCol [_nthToPat nth]
    ...
    normalize (NthCol nth) = NthCol (normalizeNth nth)
    normalize (NthLastChild nth) = NthLastChild (normalizeNth nth)
    normalize (NthLastCol nth) = NthLastCol (normalizeNth nth)
...
instance Binary PseudoClass where
  ...
  put (NthCol nth) = putWord8 29 >> put nth
  put (NthLastCol nth) = putWord8 30 >> put nth
...
instance Arbitrary PseudoClass where
    arbitrary = oneof ((Lang <$> elements _arbitraryLanguages) : map pure [
      ...
      ] ++ map (<$> arbitrary) [NthChild, NthLastChild, NthLastOfType, NthOfType, NthCol, NthLastCol])
